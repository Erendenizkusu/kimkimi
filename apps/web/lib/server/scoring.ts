import { AnswerPhase, QuestionType, type RoomPlayer } from '@prisma/client';
...
export function singleChoiceBenOBiPerspectiveMatch(expected: string, answered: string): boolean {
  ...
  return (expected === 'Ben' && answered === 'O') || (expected === 'O' && answered === 'Ben');
}
...
export async function evaluateRoom(roomId: string): Promise<RoomSummary> {
  const room = await prisma.room.findUnique({
    where: { id: roomId },
    include: { roomPlayers: true, category: true },
  });
  if (!room) throw new Error('Room not found');

  const order = parseGameOrderJson(room.questionOrderJson);
  const gameQuestions = await prisma.question.findMany({ where: { id: { in: order } } });
  ...
  const profileAnswers = await prisma.answer.findMany({
    where: { phase: AnswerPhase.profile, roomPlayerId: { in: [host.id, guest.id] } },
  });
  ...
  const gameAnswers = await prisma.answer.findMany({
    where: { phase: AnswerPhase.game, roomPlayerId: { in: [host.id, guest.id] } },
  });
  ...
  const scoreFor = (player: RoomPlayer, partner: RoomPlayer): ResultDetail[] => {
    ...
    for (const qid of order) {
      ...
      const correct = compareValues(q.type, expected, answered, q.choicesJson);
      details.push({ questionId: qid, prompt: q.prompt, correct, expected, answered });
    }
    return details;
  };
